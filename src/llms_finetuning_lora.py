...
def load_data(class_to_predict, use_history, use_past_labels, use_context):
    final_path = f'./preprocessed/data{get_extension(class_to_predict, use_history, use_past_labels, use_context)}'
...
if __name__ == '__main__':
    ...
    dataset = dataset.map(lambda x: {"formatted_text": tokenizer.apply_chat_template(x["text"], tokenize=False, add_generation_prompt=True)})
    ...
    answers = []
    for text in tqdm.tqdm(dataset['test']['formatted_text']):
    ...
    np.save(f'./pred_results/mistral{get_extension(class_to_predict, use_history, use_past_labels, use_context)}.npy', answers)
